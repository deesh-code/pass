...
@st.cache_data
def get_student_applications(student_id, status):
...
def student_portal():
    ...
    try:
        applications = get_student_applications(student_id, "challenge sent")
        if applications:
            ...
            for app in applications:
                ...
                if challenges:
                    ...
                    # Proctoring
                    proctoring_js = load_proctoring_js()
                    ...
                    if st.button("Submit Answers", key=f"submit_{challenge['$id']}"):
                        with st.spinner("Scoring your answers..."):
                            ...
                            # Score the challenge
                            challenge_score = score_challenge(challenge['questions'], answers)
                            ...
                            databases.update_document(DB_ID, CHALLENGES_COLLECTION_ID, challenge['$id'], {"results": json.dumps(results_data)})
                            databases.update_document(DB_ID, APPLICATIONS_COLLECTION_ID, app['$id'], {"status": "challenge scored"})
                            st.success(f"Your answers have been submitted and scored! Your score is {challenge_score}.")
                            if challenge_score >= 80:
                                st.balloons()
                                st.info("Congratulations! You have passed the challenge. The HR team will be in touch with you shortly.")
                            else:
                                st.warning("You have not passed the challenge. We encourage you to apply for other jobs.")
